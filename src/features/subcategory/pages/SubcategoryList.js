...
import { getByIDSubCategory } from '../subcategoryAction';
    ...
    >
      ...
      {numSelected > 0 ? (
        ...
        <Button color="success" variant="contained" endIcon={<AddIcon />} >
          <Link to="/app/subcategory-create">Add</Link>
        </Button>
...
const SubcategoryList = (props) => {
  ...
  const subCategoryLists = props.subCategoryLists
  const {userToken} = useSelector(state => state.auth)
  const {loading , subCategoryInfo , message , success} = useSelector((state)=> state.subCategory) 
...
const handleSelectAllClick = (event) => {
  if (event.target.checked) {
    const newSelected = subCategoryLists.map((n) => n._id);
...
const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - subCategoryLists.length) : 0;

const visibleRows = React.useMemo(
  () => subCategoryLists.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage),
  [order, orderBy, page, rowsPerPage]
);

  return (
    <>
    <TitleCard title="Sub-Category Lists" topMargin="mt-2">
        ...
        <TableContainer>
          <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle" size={dense ? 'small' : 'medium'}>
            <SubcategoryTableHead
              ...
              rowCount={subCategoryLists.length}
